  ...
  return (
    <>
      ...
      >
        ...
        {/* Toast Notification — slides in from right (hero easter eggs only) */}
        <AnimatePresence>
          {toastMessage && (
            <motion.div
              key={toastMessage}
              ...
              style={{
                background: 'var(--bg-secondary)',
                color: 'var(--color-green)',
                borderColor: 'rgba(91,204,126,0.4)',
                boxShadow: '0 0 30px rgba(91,204,126,0.2), 0 8px 32px rgba(0,0,0,0.3)',
              }}
            >
              <div className="flex items-center gap-2">
                <span>✓</span>
...
function DataDashboard({ overdrive, onToast }: { overdrive: boolean; onToast: (msg: string) => void }) {
  ...
  const frontAnim = { rotateZ: 0, x: 0, y: 0, scale: 1, opacity: 1 };
        ...
        <div className="col-span-2 p-4" style={{ background: 'var(--bg-secondary)' }}>
          ...
          <div className="space-y-2">
            {pipelineStates.map((item, i) => (
              <motion.button key={item.name} className="flex items-center justify-between w-full text-left rounded-md px-2 py-1.5 -mx-2 hover:bg-[rgba(255,255,255,0.05)] transition-colors cursor-pointer" onClick={() => togglePipelineStatus(i)} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.1 }} whileTap={{ scale: 0.97 }}>
        ...
        <div className="col-span-2 p-4" style={{ background: 'var(--bg-secondary)' }}>
          ...
          <div className="space-y-2">
            {stageStates.map((item, i) => (
              <motion.button key={item.name} className="flex items-center justify-between w-full text-left rounded-md px-2 py-1.5 -mx-2 hover:bg-[rgba(255,255,255,0.05)] transition-colors cursor-pointer" onClick={() => toggleStage(i)} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.1 }} whileTap={{ scale: 0.97 }}>
                <span className="text-[11px] font-[family-name:var(--font-jetbrains)] text-[var(--text-primary)]">{item.name}</span>
                <div className="flex items-center gap-1.5">
                  <motion.div className="w-1.5 h-1.5 rounded-full" style={{ background: item.color }} animate={item.status === 'running' ? { opacity: [1, 0.3, 1] } : item.status === 'paused' ? { scale: [1, 1.3, 1] } : item.status === 'error' ? { opacity: [1, 0, 1] } : {}} transition={{ duration: item.status === 'error' ? 0.5 : 1.5, repeat: Infinity }} />
                  <span className="text-[9px] font-[family-name:var(--font-jetbrains)]" style={{ color: item.color }}>{item.status}</span>
